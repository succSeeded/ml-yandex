...
template<typename T> size_t subseq_len(const std::vector<T>& upper_seq, const std::vector<T>& lower_seq)
{
    ...
    for (size_t i = 1; i < (n+1); i++)
    {
        ...
        {
            table[(i % 2) * (m + 1) + j] = std::max(table[(i % 2) * (m + 1) + j - 1], table[((i + 1) % 2) * (m + 1) + j]);
            eq_currs = table[((i + 1) % 2) * (m + 1) + j - 1] + 1;
            if (upper_seq[i-1] == lower_seq[j-1])
            {
                table[(i % 2) * (m + 1) + j] = std::max(table[(i % 2) * (m + 1) + j], eq_currs);
            }
        }
    }
